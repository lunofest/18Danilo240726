...
function updateTimer() {
  const targetDate = new Date("july 24, 2026 21:00:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const phoneNumber1 = '542964611776'; // Número para el primer botón
...
function buscarMesa() {
  ...
  if (nombreBuscado !== "") {
    ...
    if (mesaInfo !== null) {
      ...
      resultadoDiv.classList.remove("animar"); // Quitar animación previa si existe
      resultadoDiv.innerText = texto;
      resultadoDiv.style.display = "block"; // Mostrar el div
      // Forzar reflow para reiniciar animación si se muestra varias veces
      ...
      // Ocultar resultado tras 5 segundos con animación inversa
      setTimeout(() => {
        resultadoDiv.classList.remove("animar");
        // Esperar a que termine transición de ocultar
        setTimeout(() => {
...
function obtenerInformacionMesa(nombre) {
  ...
  var invitados = [
    ...
    // INVITADOS PARA 1 PERSONA 
    { nombre: "Albornoz Enzo", textoLibre: "Tu invitacion corresponde para 1 persona" },
    { nombre: "Amaya Joaquín", textoLibre: "Tu invitacion corresponde para 1 persona" },
    { nombre: "Alfaro Leandro", textoLibre: "Tu invitacion corresponde para 1 persona" },
    { nombre: "Almada Alejo", textoLibre: "Tu invitacion corresponde para 1 persona" },
...
document.addEventListener('DOMContentLoaded', function () {
  // Definir los números de teléfono
  const recipientNumber1 = '542964611776'; // Número para el primer botón
